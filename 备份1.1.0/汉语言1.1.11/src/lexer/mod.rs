...
impl Lexer {
    ...
    fn is_comment_start(&self) -> bool {
        ...
        // 支持 "#" 格式
        let is_hash = self.current_char() == "#";
        
        is_zhushi || is_double_slash || is_hash
    }
    ...
    fn try_parse_keyword(&mut self) -> LexerResult<Option<Token>> {
        ...
        for (keyword, kw_type) in keywords.iter() {
            if self.match_string(keyword) {
                ...
                let next_pos = self.pos + keyword_len;
                let is_end_of_input = next_pos >= self.chars.len();
                let is_boundary = is_end_of_input || {
                    let next_char = &self.chars[next_pos];
                    let next_char_str: &str = next_char.as_str();
                    let is_whitespace = next_char_str == " " || next_char_str == "\t" || next_char_str == "\n" || next_char_str == "\r";
                    let is_delimiter = ["（", "）", "［", "］", "【", "】", "｛", "｝", "，", "：", "；", "．", "·",
                        "(", ")", "[", "]", "{", "}", ",", ":", ""].contains(&next_char_str);
                    let is_operator = ["＋", "－", "×", "÷", "％", "＝", "＞", "＜", "！",
                        "+", "-", "*", "/", "%", "=", ">", "<", ""].contains(&next_char_str);
                    is_whitespace || is_delimiter || is_operator
                };
