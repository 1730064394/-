...
use crate::runtime::value::{Environment, Value, PromiseStatus};
use crate::stdlib::{
...
impl Interpreter {
    ...
    fn execute_statement(&mut self, stmt: &Statement, env: Rc<RefCell<Environment>>) -> RuntimeResult<Value> {
        match stmt {
            Statement::变量定义 { 名称, 类型, 初始值 } => {
                let value = if let Some(expr) = 初始值 {
                    let evaluated_value = self.evaluate(expr, Rc::clone(&env))?;
                    // 类型检查
                    if let Some(type_name) = 类型 {
                        self.check_type(&evaluated_value, type_name)?;
                    }
                    evaluated_value
                } else {
                    Value::空值
            ...
            Statement::函数定义 { 名称, 参数, 可变参数名, 返回类型, 函数体, 异步, 装饰器 } => {
                ...
                self.functions.insert(名称.clone(), (param_names, 函数体.clone()));
                ...
                // 应用装饰器
                for decorator_expr in 装饰器.iter().rev() {
                    let decorator = self.evaluate(decorator_expr, Rc::clone(&env))?;
                    match decorator {
                        Value::函数 { 名称: _, 参数: _, 默认值: _, 可变参数名: _, 闭包: _ } => {
                            // 调用装饰器函数，传入原函数
                            let args = vec![func.clone()];
                            let result = self.call_function(&decorator, &args, Rc::clone(&env))?;
                            func = result;
                        }
                        _ => {
                            return Err(RuntimeError::TypeError(
                                format!("装饰器必须是函数，实际得到 {}", decorator.type_name())
                            ));
                        }
                    }
                }
            ...
            Statement::导入语句 { 模块名, 别名 } => {
                ...
                // 检查模块缓存
                if let Some(module) = self.module_cache.get(模块名) {
                    env.borrow_mut().define(import_name.clone(), module.clone());
                    return Ok(Value::空值);
                }
                ...
                for path in &search_paths {
                    ...
                    // 在子目录中查找（用于packages目录结构）
                    let module_file_subdir = format!("{}/{}/{}.hy", path, 模块名, 模块名);
                    if let Ok(c) = std::fs::read_to_string(&module_file_subdir) {
                        module_content = Some(c);
                        break;
                    }
                    let module_file_subdir_zh = format!("{}/{}/{}.汉", path, 模块名, 模块名);
                    if let Ok(c) = std::fs::read_to_string(&module_file_subdir_zh) {
                        module_content = Some(c);
                        break;
                    }
                }
                ...
                // 缓存模块
                self.module_cache.insert(模块名.clone(), module.clone());
            ...
            Statement::从导入语句 { 模块名, 成员, 别名 } => {
                // 检查模块缓存
                let module_value = if let Some(module) = self.module_cache.get(模块名) {
                    module.clone()
                } else {
                    let mut module_content: Option<String> = None;
                ...
                // 从模块值中提取成员
                match module_value {
                    Value::字典(module_props) => {
                        if let Some(alias) = 别名 {
                            // 从 模块 导入 成员 为 别名
                            if 成员.len() != 1 {
                                return Err(RuntimeError::General("当使用别名时，只能导入一个成员".to_string()));
                            }
                            let member_name = &成员[0];
                            let member_value = module_props.get(member_name)
                                .ok_or_else(|| RuntimeError::General(format!("模块 {} 中不存在成员: {}", 模块名, member_name)))?;
                            env.borrow_mut().define(alias.clone(), member_value.clone());
                        } else {
                            // 从 模块 导入 成员1, 成员2, ...
                            for member_name in 成员 {
                                let member_value = module_props.get(member_name)
                                .ok_or_else(|| RuntimeError::General(format!("模块 {} 中不存在成员: {}", 模块名, member_name)))?;
                                env.borrow_mut().define(member_name.clone(), member_value.clone());
                            }
                        }
                    }
                    _ => {
                        return Err(RuntimeError::General(format!("模块 {} 不是有效的模块对象", 模块名)));
                    }
                }
            ...
            Statement::类定义 { 名称, 父类, 成员, 装饰器, 元类 } => {
                ...
                let mut 元类_value = None;
                if let Some(meta_expr) = 元类 {
                    元类_value = Some(self.evaluate(meta_expr, Rc::clone(&env))?);
                    // 确保元类是一个类
                    if let Some(meta) = &元类_value {
                        if !matches!(meta, Value::类 { .. }) {
                            return Err(RuntimeError::TypeError(
                                format!("元类必须是一个类，实际得到 {}", meta.type_name())
                            ));
                        }
                    }
                }
                ...
                for member in 成员 {
                    match member {
                        ClassMember::方法 { 名称: method_name, 参数, 返回类型: _, 函数体, 访问权限, 装饰器: method_decorators } => {
                            let default_values: Vec<Option<Expression>> = 参数.iter().map(|p| p.默认值.clone()).collect();
                            let closure = env.borrow().clone();
                            
                            let mut method = Value::函数 {
                            ...
                            // 应用装饰器
                            for decorator_expr in method_decorators.iter().rev() {
                                let decorator = self.evaluate(decorator_expr, Rc::clone(&env))?;
                                match decorator {
                                    Value::函数 { 名称: _, 参数: _, 默认值: _, 可变参数名: _, 闭包: _ } => {
                                        // 调用装饰器函数，传入原函数
                                        let args = vec![method.clone()];
                                        let result = self.call_function(&decorator, &args, Rc::clone(&env))?;
                                        method = result;
                                    }
                                    _ => {
    ...
    fn evaluate(&mut self, expr: &Expression, env: Rc<RefCell<Environment>>) -> RuntimeResult<Value> {
        match expr {
            ...
            Expression::匿名函数 { 参数, 可变参数名, 函数体 } => {
                ...
                self.functions.insert(func_name.clone(), (param_names, 函数体.clone()));
            ...
            Expression::Await表达式 { 表达式 } => {
                let value = self.evaluate(表达式, env)?;
                match value {
                    Value::Promise { 状态, 值, 错误 } => {
                        match 状态 {
                            PromiseStatus::已完成 => {
                                if let Some(resolved_value) = 值 {
                                    Ok(*resolved_value)
                                } else {
                                    Ok(Value::空值)
                                }
                            ...
                            PromiseStatus::已拒绝 => {
                                if let Some(error_value) = 错误 {
                                    Err(RuntimeError::General(format!("Promise拒绝: {:?}", error_value)))
                                } else {
                                    Err(RuntimeError::General("Promise拒绝".to_string()))
                                }
                            }
                            PromiseStatus::待处理 => {
                                // 简单实现：直接返回Promise
                                Ok(Value::Promise { 状态, 值, 错误 })
                            }
                        }
                    }
                    _ => {
    ...
    fn unary_op(&self, op: &UnaryOperator, operand: &Value) -> RuntimeResult<Value> {
        match op {
            ...
            UnaryOperator::解引用 => match operand {
                Value::列表(_) | Value::字典(_) | Value::集合(_) => Ok(operand.clone()),
                _ => Err(RuntimeError::TypeError(format!(
                    "类型 {} 无法解引用",
                    operand.type_name()
                ))),
        ...
        match func {
            ...
            Value::函数 { 名称, 参数, 默认值, 可变参数名, 闭包, .. } => {
                ...
                if let Some(body) = body {
                    ...
                    // 绑定参数
                    for (i, param) in 参数.iter().enumerate() {
                        let arg_value = if i < args.len() {
                            let value = args[i].clone();
                            // 检查参数类型注解
                            if let Some(param_type) = &param.类型 {
                                self.check_type(&value, param_type)?;
                            }
                            value
                        } else if let Some(Some(ref default_expr)) = 默认值.get(i) {
                            let value = self.evaluate(default_expr, Rc::new(RefCell::new(闭包.clone())))?;
                            // 检查参数类型注解
                            if let Some(param_type) = &param.类型 {
                                self.check_type(&value, param_type)?;
                            }
                        ...
                        // 直接使用参数的名称进行绑定
                        func_env.define(param.名称.clone(), arg_value);
                    }
            ...
            Value::异步函数 { 名称, 参数, 默认值, 可变参数名, 闭包, .. } => {
                ...
                if let Some(body) = body {
                    ...
                    let promise = Value::Promise {
                        状态: PromiseStatus::已完成,
                        值: Some(Box::new(result)),
                        错误: None,
                    };
                    Ok(promise)
                } else {
            ...
            Value::类 { .. } => {
                // 调用类创建实例
                self.create_object(func, args, env)
            }
            _ => Err(RuntimeError::TypeError(format!(
                "类型 {} 不可调用",
                func.type_name()
    ...
    fn call_method(
        ...
        match (obj, method_name) {
            ...
            (Value::字典(m), method_name) => {
                if let Some(func) = m.get(method_name) {
                    match func {
                        Value::内置函数 { 函数, .. } => 函数(args.to_vec()),
                        Value::函数 { 名称, 参数, 默认值, 可变参数名, 闭包, .. } => {
                            let min_args = 参数.len() - 默认值.iter().filter(|d| d.is_some()).count();
            ...
            (Value::对象 { 类名, 属性, .. }, method_name) => {
                ...
                if let Some(body) = method_body {
                    // 创建方法环境，并将当前环境作为父环境
                    let mut method_env = Environment::with_parent(env.borrow().clone());
            ...
            (Value::类 { 名称: class_name, 方法, .. }, method_name) => {
                let method_body = self.class_methods.get(class_name)
                    .and_then(|methods| methods.get(method_name).cloned());
                ...
                if let Some(body) = method_body {
                    ...
                    // 尝试获取方法的参数信息
                    if let Some(Value::函数 { 参数: func_params, .. }) = 方法.get(method_name) {
                        // 将传入的参数绑定到对应的参数名上
                        for (i, (param, arg)) in func_params.iter().zip(args.iter()).enumerate() {
                            method_env.define(param.名称.clone(), arg.clone());
                        }
                        // 对于额外的参数，仍然使用参数0、参数1等命名
                        for (i, arg) in args.iter().enumerate().skip(func_params.len()) {
                            method_env.define(format!("参数{}", i), arg.clone());
                        }
                    } else {
                        // 如果无法获取参数信息，使用默认的参数0、参数1等命名
                        for (i, arg) in args.iter().enumerate() {
                            method_env.define(format!("参数{}", i), arg.clone());
                        }
                    }
    ...
    fn get_property(&self, obj: &Value, property_name: &str) -> RuntimeResult<Value> {
        match obj {
            ...
            Value::类 { 属性默认值, .. } => 属性默认值
                .get(property_name)
                .cloned()
                .ok_or_else(|| RuntimeError::General(format!("类没有属性 {}", property_name))),
    ...
    fn assign_to(
        ...
        match target {
            Expression::标识符 { 名称 } => {
                if !env.borrow_mut().set(名称, value.clone()) {
                    // 如果变量不存在，定义一个新变量
                    env.borrow_mut().define(名称.clone(), value);
                }
                Ok(Value::空值)
    ...
    fn create_object(
        ...
        match class {
            Value::类 { 名称, 方法, 属性默认值, 属性权限, 元类, .. } => {
                // 检查是否有元类
                if let Some(meta) = 元类 {
                    // 元类逻辑：先调用元类的创建方法，再调用初始化方法
                    if let Value::类 { .. } = &**meta {
                        // 首先调用元类的 创建 方法
                        let mut obj = match self.call_method(&**meta, "创建", &[class.clone()], env.clone()) {
                            Ok(result) => result,
                            Err(_) => {
                                // 如果没有创建方法，创建一个默认对象
                        ...
                        // 然后调用元类的 初始化 方法
                        let _ = self.call_method(&**meta, "初始化", &[obj.clone()], env.clone());
                        
                        // 调用普通类的 初始化 方法
                        if let Some(init_method) = 方法.get("初始化") {
                            let _ = self.call_method(&obj, "初始化", args, env.clone());
                        }
    ...
    fn check_type(&self, value: &Value, expected_type: &str) -> RuntimeResult<()> {
        use std::collections::HashMap;
        
        let actual_type = value.type_name();
        
        // 类型映射
        let type_mapping = HashMap::from([
            ("整数", "整数"),
            ("浮点数", "浮点数"),
            ("字符串", "字符串"),
            ("布尔值", "布尔值"),
        ...
        if let Some(mapped_type) = type_mapping.get(expected_type) {
