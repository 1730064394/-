...
pub enum Value {
    ...
    函数 {
        名称: String,
        参数: Vec<crate::parser::ast::FunctionParameter>,
...
impl Value {
    ...
    pub fn to_string_value(&self) -> String {
        match self {
            ...
            Value::Promise { 状态, 值, .. } => {
                match 状态 {
                    PromiseStatus::待处理 => String::from("<Promise (待处理)>"),
                    PromiseStatus::已完成 => format!("<Promise (已完成): {:?}>", 值),
                    PromiseStatus::已拒绝 => String::from("<Promise (已拒绝)>"),
                }
            }
        }
    }
