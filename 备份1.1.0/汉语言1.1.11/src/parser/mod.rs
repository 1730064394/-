...
impl Parser {
    ...
    fn parse_statement(&mut self) -> ParserResult<Option<Statement>> {
        ...
        while self.check_delimiter(Delimiter::艾特) {
            self.advance();
        ...
        if self.check_keyword(Keyword::异步) {
            self.advance();
            if self.check_keyword(Keyword::定义) {
                let stmt = self.parse_definition()?;
                // 如果是函数定义，设置异步标志和装饰器
                if let Some(Statement::函数定义 { 名称, 参数, 可变参数名, 返回类型, 函数体, .. }) = stmt {
                    return Ok(Some(Statement::函数定义 {
                        名称,
                        参数,
                        可变参数名,
                        返回类型,
                ...
                return Err(ParserError::UnexpectedToken {
                    expected: "定义".to_string(),
                    actual: self.current_token().to_string(),
                    line: self.current_token().line,
                    column: self.current_token().column,
                });
            }
        } else if self.check_keyword(Keyword::定义) {
            let stmt = self.parse_definition()?;
            // 如果是函数定义，设置装饰器
            if let Some(Statement::函数定义 { 名称, 参数, 可变参数名, 返回类型, 函数体, 异步, .. }) = stmt {
                return Ok(Some(Statement::函数定义 {
                    名称,
                    参数,
                    可变参数名,
        ...
        } else if self.check_keyword(Keyword::导入) || self.check_keyword(Keyword::从) {
            self.parse_import_statement()
        } else if self.check_keyword(Keyword::类) {
            let stmt = self.parse_class_definition()?;
            // 如果是类定义，设置装饰器
            if let Some(Statement::类定义 { 名称, 父类, 成员, 元类, .. }) = stmt {
                return Ok(Some(Statement::类定义 {
                    名称,
                    父类,
                    成员,
                    装饰器: decorators,
    ...
    fn parse_definition(&mut self) -> ParserResult<Option<Statement>> {
        ...
        self.skip_newlines();
        if self.check_keyword(Keyword::函数) || self.check_delimiter(Delimiter::左括号) {
            if self.check_keyword(Keyword::函数) {
                self.advance();
                self.skip_newlines();
            }
    ...
    fn parse_function_definition(&mut self, name: String) -> ParserResult<Option<Statement>> {
        ...
        // 函数关键字已经被parse_definition消耗了，不需要再次检查
        ...
        if self.check_operator(Operator::箭头) {
            self.advance();
            return_type = Some(self.consume_identifier()?);
        } else if self.check_keyword(Keyword::为) || self.check_keyword(Keyword::是) {
            self.advance();
            return_type = Some(self.consume_identifier()?);
        }
    ...
    fn parse_import_statement(&mut self) -> ParserResult<Option<Statement>> {
        if self.check_keyword(Keyword::从) {
            // 解析 "从 模块 导入 成员" 语法
            self.advance();
    ...
    fn parse_class_definition(&mut self) -> ParserResult<Option<Statement>> {
        ...
        let mut 元类 = None;
        if self.check_keyword(Keyword::元类) {
            self.advance();
            元类 = Some(self.parse_expression()?);
        }
        ...
        while !self.is_at_end() && !self.check_keyword(Keyword::结束) {
            ...
            if self.check_keyword(Keyword::定义) || self.check_keyword(Keyword::变量) || self.check_keyword(Keyword::属性) || self.check_keyword(Keyword::方法) || self.check_keyword(Keyword::类变量) {
                let is_method = self.check_keyword(Keyword::方法);
                if self.check_keyword(Keyword::定义) || self.check_keyword(Keyword::变量) || self.check_keyword(Keyword::属性) || self.check_keyword(Keyword::类变量) {
                    self.advance();
                ...
                if self.check_keyword(Keyword::函数) || is_method {
                    ...
                    if let Some(Statement::函数定义 { 参数, 返回类型, 函数体, 装饰器, .. }) = func_stmt {
                        // 合并方法装饰器和函数装饰器
                        let mut all_decorators = method_decorators;
                        all_decorators.extend(装饰器);
    ...
    fn parse_try_statement(&mut self) -> ParserResult<Option<Statement>> {
        ...
        let try_body = if self.check_delimiter(Delimiter::左花括号) {
            self.advance(); // 消耗 {
            let stmts = self.parse_block()?;
            self.consume_delimiter(Delimiter::右花括号)?;
            stmts
        ...
        while self.check_keyword(Keyword::捕获) {
            ...
            let catch_body = if self.check_delimiter(Delimiter::左花括号) {
                self.advance(); // 消耗 {
                let stmts = self.parse_block()?;
                self.consume_delimiter(Delimiter::右花括号)?;
                stmts
    ...
    fn parse_unary(&mut self) -> ParserResult<Expression> {
        ...
        if self.check_operator(Operator::乘) {
            ...
            let operand = self.parse_unary()?;
            return Ok(Expression::一元运算 {
                运算符: UnaryOperator::解引用,
                操作数: Box::new(operand),
            });
        }
        
        if self.check_keyword(Keyword::等待) {
            self.advance();
            let expr = self.parse_postfix()?;
            return Ok(Expression::Await表达式 {
                表达式: Box::new(expr),
            });
        }
    ...
    fn parse_postfix(&mut self) -> ParserResult<Expression> {
        ...
        loop {
            if self.check_delimiter(Delimiter::左括号) {
                ...
                self.advance();
                let attr_name = match &self.current_token().value {
                    TokenValue::Identifier(name) => {
                        let name = name.clone();
                        self.advance();
                        name
