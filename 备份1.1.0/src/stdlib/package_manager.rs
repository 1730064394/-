...
const DEFAULT_GITHUB_REPO: &str = "https://github.com/1730064394/---";
...
fn 下载库(库名称: &str, 本地路径: &str, 源仓库: &str) -> RuntimeResult<Value> {
    ...
    let 示例文件路径 = 包目录.join(format!("{}.hy", 库名称));
    let 示例内容 = format!(
        "// {} 库示例文件\n// 自动下载自: {}\n\n打印(\"欢迎使用 {} 库!\")\n",
        库名称, 源仓库, 库名称
    );
    fs::write(&示例文件路径, 示例内容)
        .map_err(|e| RuntimeError::General(format!("创建示例文件失败: {}", e)))?;
